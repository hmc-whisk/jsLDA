...
function readMallet(serializedModel: string, model: LDAModel) {
    ...
    serializedModel.split('\n').forEach((line, i) => {
        if (line.startsWith("#alpha")) {
            ...
            if (Number.isNaN(beta)) throw Error(`Cannot parse beta (line ${i + 1})`)
            ...
            for (let v of [sDocIndex, _, sTokenIndex, sTypeIndex, token, sTopic])
                if (v === undefined) throw Error(`Cannot parse line ${i + 1} in state file` )
            ...
            for (let v of [docIndex, tokenIndex, typeIndex, topic])
                ...
                if (!(v >= 0)) throw Error(`Cannot parse line ${i + 1} in state file` )
    ...
    if (alpha.length !== model.numTopics) throw Error(`Length of alpha vector ${alpha.length} does not match number of topics in model (${model.numTopics}, inferred from length of annotations)`)
 ...
 * @param documents: a compatible csv or tsv documents file
 * @param documentsType: the MIME type of documents file
 * @param stopwords: a newline separated list of stopwords
 * @param state: a MALLET state file
 */
async function reconstructModel(annotations: string[], documents: string, documentsType: string, stopwords: string, state: string): Promise<LDAModel> {
    ...
    let deserializedModel = new LDAModel(annotations.length);
    deserializedModel.annotations = annotations
    ...
    readMallet(state, deserializedModel)
...
export async function deserializeModel(serializedModel: Blob): Promise<LDAModel> {
    ...
    return reconstructModel(deserializedAnnotations, documents, documentsType, stopwords, model);
}

export async function deserializeMalletUpload(annotations: string[], documents: string, documentsType: string, stopwords: string, state: string): Promise<LDAModel> {
    return reconstructModel(annotations, documents, documentsType, stopwords, state)
}

export async function readDocumentsUpload(nTopics: number, documents: string, contentType: string): Promise<LDAModel> {
    let model = new LDAModel(nTopics)
    model.setDocumentType(contentType)
    model._parseDoc(documents)
    model.sortTopicWords()
    return model
}
